...
namespace ff{
  ...
  class thread_local_var
  {
    public:
      ...
      inline auto for_each(FT && f) -> typename std::enable_if<
        utils::is_function_with_arg_type<FT, T>::value, void>::type
      {
      ...
      auto for_each(F && f) -> typename std::enable_if<
        ...
        void>::type{
          static_assert(Please_Check_The_Assert_Msg<F>::value, FF_EM_CALL_FOR_EACH_WITH_NO_FUNC);
      }
      ...
      auto for_each(F && f) ->typename std::enable_if<
        ...
        void>::type{
          static_assert(Please_Check_The_Assert_Msg<F>::value, FF_EM_CALL_FOR_EACH_WITH_WRONG_PARAM);
        }
