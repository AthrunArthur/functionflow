...
static void for_each_impl_general_iterator(Iterator_t begin, Iterator_t end, Functor_t && f, Entities_t & es, auto_partitioner * p)
{
    ...
    for_each_impl_general_iterator_auto_partition(begin, end, std::forward<Functor_t>(f), es, count, divide_times);
}

template<class Iterator_t, class Functor_t>
static void for_each_impl_general_iterator_auto_partition(Iterator_t begin, Iterator_t end, Functor_t && f,
        Entities_t & es, size_t count, size_t divide_times)
{
    ...
    while(divide_times !=0 && left != 1)
    {
        ...
        p([bt, t, sc, &f, &es, divide_times](){
            for_each_impl_general_iterator_auto_partition(bt, t, std::move(f), es, sc, divide_times -1);});
        lgroup.push_back(p);
        divide_times --;
    }
