...
注意，类似于para对象，paragroup也可以使用依赖表达式表示其依赖关系。同时paragroup可以通过any, all作为依赖表达式的子表达式存在。

##4.1数据并行
paragroup对象的for_each方法用于表达数据并行，原型如下：

	template<class Iterator_t, class Functor_t>
	for_each(Iterator_t begin, Iterator_t end, Functor_t && f);
...
accumulator典型的使用场景是对变量进行累计操作，例如求和操作，一个简单的例子如下：
	...
	ff::paragroup pg;
	pg.for_each(s.begin(), s.end(), [&sum](int x){
	  sum.increase(x);
	});
	ff_wait(all(pg));
	std::cout<<"pg sum: "<<sum.get()<<std::endl;
...
#FF的更多后续工作
FF将会添加更多任务级的同步原语，例如锁，条件变量，等，以及设计阻塞的I/O操作等，敬请期待！